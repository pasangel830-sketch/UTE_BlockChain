...
export function createMockCtx(
  ...
  const tmap = new Map<string, Uint8Array>();
  for (const [k, v] of Object.entries(transient)) {
    tmap.set(k, Buffer.from(v, 'utf8'));
  }
  ...
  const stub = {
    ...
    getPrivateData: jest.fn(async (collection: string, key: string) => {
      return priv.get(`${collection}:${key}`) ?? Buffer.alloc(0);
    }),
    putPrivateData: jest.fn(async (collection: string, key: string, value: Uint8Array) => {
      priv.set(`${collection}:${key}`, Buffer.from(value));
    }),
    getTransient: jest.fn(() => tmap),
