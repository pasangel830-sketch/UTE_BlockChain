...
import { createMockCtx, mockHitoCompletado } from './mock-ctx';
...
describe('PagoContract', () => {
  ...
  async function custodia(
    pagoId: string,
    hitoId: string,
    empresa: string,
    importe: string,
  ): Promise<string> {
    mockHitoCompletado(ctx, hitoId, empresa, Number(importe));
    return cc.ponerEnCustodia(ctx, pagoId, hitoId, empresa, importe, '');
  }
  ...
  test('ponerEnCustodia deja fondos en CUSTODIA y reparte', async () => {
    const pago = parse<Pago>(await custodia('pago-H1', 'H1', 'EmpresaA', '10000'));
  ...
  test('completarHito dispara lógica de pago: custodia con hitoId', async () => {
    const pago = parse<Pago>(await custodia('pago-H99', 'H1-completado', 'EmpresaA', '1000'));
  ...
  test('no custodia si el hito no está COMPLETADO', async () => {
    mockHitoCompletado(ctx, 'H1', 'EmpresaA', 100, 'VALIDACION');
    await expect(cc.ponerEnCustodia(ctx, 'pago-H1', 'H1', 'EmpresaA', '100', '')).rejects.toThrow(
      /no está COMPLETADO/,
    );
  });

  test('origen completarHito no consulta el hito', async () => {
    (ctx.stub.invokeChaincode as jest.Mock).mockResolvedValue({
      status: 500,
      message: 'INVOKE_CHAINCODE failed: transaction ID: abc(channel-obra) exists',
      payload: Buffer.alloc(0),
    });
    const pago = parse<Pago>(
      await cc.ponerEnCustodia(ctx, 'pago-H1', 'H1', 'EmpresaA', '10000', 'completarHito'),
    );
    expect(pago.estado).toBe('CUSTODIA');
    expect(pago.importeTotal).toBe(10000);
    expect(ctx.stub.invokeChaincode).not.toHaveBeenCalled();
  });

  test('no duplica pago ni hito', async () => {
    await custodia('pago-H1', 'H1', 'EmpresaA', '100');
    await expect(cc.ponerEnCustodia(ctx, 'pago-H1', 'H1', 'EmpresaA', '100', '')).rejects.toThrow(
      /ya existe/,
    );
  ...
  test('autorizarPago emite PagoAutorizado y sale de CUSTODIA', async () => {
    await custodia('pago-H1', 'H1', 'EmpresaA', '10000');
  ...
  test('listarPagos paginado', async () => {
    await custodia('pago-1', 'H1', 'EmpresaA', '100');
    await custodia('pago-2', 'H2', 'EmpresaA', '200');
