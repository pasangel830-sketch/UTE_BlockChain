...
| Escrow / fondos custodiados (§4.2) | PagoContract + API; `completarHito` → `invokeChaincode` pago (11 sep, en `develop`) | HECHO (día 5; mismo tx 11 sep, Jest; red vieja: `make deploy-cc`) | funcional |
...
| Evidencias (foto/PDF) + hash | `POST /hitos/:id/evidencias` y `/incidencias/:id/evidencias`; SHA-256 en `hashEvidencia` (hito) o `notasTecnicas` PDC; binario fuera de cadena; `GET /evidencias` | HECHO (11 sep, en `develop`) | funcional |
| Estado de obra | EstadoObraContract alimentado por backend (sin cross-cc) | HECHO (día 9) | |
| API JWT, multer, Swagger, prom-client | `backend/` | HECHO (día 6; evidencias hito/incidencia 11 sep; `submitCommit` + bloque 13 sep) | |
| Frontend 7 pantallas + Explorer | `frontend/` | HECHO (días 7 y 9; detalle txs y 5 sesiones 31 ago; evidencias y fechas 11 sep; Explorer génesis 13 sep) | |
